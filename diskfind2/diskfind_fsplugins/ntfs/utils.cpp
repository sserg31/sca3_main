...
ATTR_RECORD * find_attribute(const ATTR_TYPES type, ntfs_attr_search_ctx *ctx)
{
	if (!ctx) {
        ...
        if (ntfs_attr_lookup(type, NULL, 0, (IGNORE_CASE_BOOL)0, 0, NULL, 0, ctx) != 0) {
...
struct mft_search_ctx * mft_get_search_ctx(ntfs_volume *vol)
{
	...
	if (!vol) {
        ...
        ctx = (mft_search_ctx*)calloc(1, sizeof *ctx);
	...
	for (ctx->mft_num++; (s64)ctx->mft_num < nr_mft_records; ctx->mft_num++) {
		...
		if (in_use) {
			...
			if (ctx->flags_search & FEMR_DIR) {
				...
				if (attr_ctx) {
                                        if (ntfs_attr_lookup(AT_INDEX_ROOT, NTFS_INDEX_I30, 4, (IGNORE_CASE_BOOL)0, 0, NULL, 0, attr_ctx) == 0)
