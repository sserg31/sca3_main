...
void fs_write(loff_t pos,int size,void *data)
{
    ...
    new1 = (CHANGE *)alloc(sizeof(CHANGE));
    new1->pos = pos;
    memcpy(new1->data = alloc(new1->size = size),data,size);
    new1->next = NULL;
    if (last) last->next = new1;
    else changes = new1;
    last = new1;
}
...
static void fs_flush(void)
{
    ...
    while (changes) {
        ...
        if (llseek(fd,this1->pos,0) != this1->pos)
            fprintf(stderr,"Seek to %lld failed: %s\n  Did not write %d bytes.\n",
              (long long)this1->pos,strerror(errno),this1->size);
        else if ((size = write(fd,this1->data,this1->size)) < 0)
                fprintf(stderr,"Writing %d bytes at %lld failed: %s\n",this1->size,
                  (long long)this1->pos,strerror(errno));
            else if (size != this1->size)
                    fprintf(stderr,"Wrote %d bytes instead of %d bytes at %lld."
                      "\n",size,this1->size,(long long)this1->pos);
        free(this1->data);
        free(this1);
    }
}
