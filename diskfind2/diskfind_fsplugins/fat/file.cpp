...
FD_TYPE file_type(FDSC **curr,char *fixed)
{
    ...
    if ((this1 = file_find(curr,fixed))) return (*this1)->type;
    return fdt_none;
}
...
static void report_unused(FDSC *this1)
{
    FDSC *next;

    while (this1) {
        next = this1->next;
        if (this1->first) report_unused(this1->first);
        else if (this1->type != fdt_none)
                printf("Warning: did not %s file %s\n",this1->type == fdt_drop ?
                  "drop" : "undelete",file_name((unsigned char *)this1->name));
        free(this1);
        this1 = next;
    }
}
