 ...
 *				  -2 == bad cluster
 */
void set_fat(DOS_FS *fs,unsigned long cluster,unsigned long new1)
{
	...
	if (cluster & 1) {
            ...
            data[0] = ((new1 & 0xf) << 4) | (prevEntry.value >> 8);
            data[1] = new1 >> 4;
	}
	else {
            ...
            data[0] = new1 & 0xff;
            data[1] = (new1 >> 8) | (cluster == fs->clusters-1 ? 0 :
	...
	offs = fs->fat_start+cluster*2;
        *(unsigned short *) data = CT_LE_W(new1);
	size = 2;
	break;
      case 32:
        {
	     ...
	     * are not part of the cluster number. So we never touch them. */
            *(unsigned long *) data = CT_LE_L( (new1 & 0xfffffff) |
...
void reclaim_file(DOS_FS *fs)
{
    ...
    total_num_clusters = fs->clusters + 2UL;
    num_refs = (unsigned long *)alloc(total_num_clusters * sizeof(unsigned long));
