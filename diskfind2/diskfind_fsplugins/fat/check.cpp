...
loff_t alloc_rootdir_entry(DOS_FS *fs, DIR_ENT *de, const char *pattern)
{
	...
	int next_free = 0, scan;
        ...
        root = (DIR_ENT *)alloc(fs->root_entries*sizeof(DIR_ENT));
...
static void auto_rename(DOS_FILE *file)
{
	...
	for (walk = first; walk; walk = walk->next)
            if (walk != file && !strncmp((const char *)walk->dir_ent.name,(const char *)file->dir_ent.
...
static int handle_dot(DOS_FS *fs,DOS_FILE *file,int dots)
{
    ...
    name = (char *)strncmp((const char *)file->dir_ent.name,MSDOS_DOT,MSDOS_NAME) ? (char *)".." : (char *)".";
...
static int check_file(DOS_FS *fs,DOS_FILE *file)
{
    DOS_FILE *owner;
    int restart;
    unsigned long expect,curr,this1,clusters,prev,walk,clusters2;
	...
	if ((owner = get_owner(fs,curr))) {
		...
		prev = 0;
		clusters = 0;
                for (this1 = FSTART(owner,fs); this1 > 0 && this1 != -1; this1 =
                  next_cluster(fs,this1)) {
                    if (this1 == curr) {
			if (prev) set_fat(fs,prev,-1);
			else MODIFY_START(owner,0,fs);
			MODIFY(owner,size,CT_LE_L((unsigned long long)clusters*fs->cluster_size));
			if (restart) return 1;
                        while (this1 > 0 && this1 != -1) {
                            set_owner(fs,this1,NULL);
                            this1 = next_cluster(fs,this1);
			}
                        this1 = curr;
			break;
		    }
    ...
    while (*walk) {
		...
		*walk = (*walk)->next;
		continue;
            ...
            if (!strncmp((const char *)(*walk)->dir_ent.name,MSDOS_DOT,MSDOS_NAME)) dot++;
	    else dotdot++;
	}
					 ...
					 loff_t offset,FDSC **cp)
{
    ...
    new1 = (DOS_FILE*)qalloc(&mem_queue,sizeof(DOS_FILE));
    new1->lfn = lfn_get(&de);
    new1->offset = offset;
    memcpy(&new1->dir_ent,&de,sizeof(de));
    new1->next = new1->first = NULL;
    new1->parent = parent;
    if (type == fdt_undelete) undelete(fs,new1);
    **chain = new1;
    *chain = &new1->next;
    if (list) {
        printf("Checking file %s",path_name(new1));
        if (new1->lfn)
            printf(" (%s)", file_name(new1->dir_ent.name) );     /* (8.3) */
	printf("\n");
    }
	...
	++n_files;
    test_file(fs,new1,test);     /* Bad cluster check */
}
...
static int scan_dir(DOS_FS *fs,DOS_FILE *this1,FDSC **cp)
{
    DOS_FILE **chain;
    int i;
    ...
    chain = &this1->first;
    i = 0;
    clu_num = FSTART(this1,fs);
    new_dir();
    while (clu_num > 0 && clu_num != -1) {
        add_file(fs,&chain,this1,cluster_start(fs,clu_num)+(i % fs->
	  cluster_size),cp);
	i += sizeof(DIR_ENT);
    ...
    lfn_check_orphaned();
    if (check_dir(fs,&this1->first,this1->offset)) return 0;
    if (check_files(fs,this1->first)) return 1;
    return subdirs(fs,this1,cp);
}
